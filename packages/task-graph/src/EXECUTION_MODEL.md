...
- **Cross-process boundary** (`@workglow/job-queue`): by default live events are same-process only — `JobQueueWorker.emitStreamEvent` fires an in-memory `job_stream` event that a `JobQueueServer` forwards to attached clients, and `JobHandle.onStream` is present only on a server-attached handle. A **channel-capable** `IMessageQueue` (one that advertises the optional `publishStreamChunk` / `subscribeToStream`) additionally carries them out of process: the worker assigns a monotonic 1-based per-job `seq` at emit time and best-effort publishes each event (a publish failure never fails the job); a storage-only `JobQueueClient` on that queue subscribes per job with a stream listener, orders and de-duplicates rows by `seq` through a `StreamReassembler` (which also buffers gaps so a reordering carrier stays correct), and dispatches through the same `onStream` machinery. `onStream` is therefore exposed whenever the server **or** the queue's stream channel can deliver, and behaves identically regardless of transport. Server-attached clients keep the direct in-memory fast path (no channel double-delivery). Deterministic re-dispatch is unaffected: the channel is a pure observation side-stream that never influences claim / execution / retry, and a queue with no channel degrades to today's "no live stream cross-process".

`InMemoryQueueStorage` is the reference carrier — a per-job ordered append log plus live subscribers, with `seq > sinceSeq` replay for late or reconnecting subscribers and eviction of a job's stream on row deletion. Durable cluster carriers (Postgres `LISTEN/NOTIFY` signalling a `(job_id, seq, bytes)` chunk table, since NOTIFY payloads are size-capped; Supabase Realtime), the IndexedDB cross-tab `BroadcastChannel` carrier, and the base64 binary row serialization those durable carriers require are a follow-up.
