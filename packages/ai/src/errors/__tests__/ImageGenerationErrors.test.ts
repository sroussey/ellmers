...
describe("ImageGenerationErrors", () => {
  it("ProviderUnsupportedFeatureError carries field and modelId", () => {
    const err = new ProviderUnsupportedFeatureError(
      "mask",
    ...
    expect(err.message).toBe(
      `Model "google/gemini-2.5-flash-image" does not support input field "mask": mask is not supported`
    );
  });

  it("ImageGenerationContentPolicyError carries provider reason and is non-retryable", () => {
    const err = new ImageGenerationContentPolicyError(
      "openai/gpt-image-2",
    ...
    expect(err.message).toBe(
      `Image generation refused by openai/gpt-image-2: violates safety policy`
    );
  });

  it("ImageGenerationProviderError defaults to retryable", () => {
    const err = new ImageGenerationProviderError("openai/gpt-image-2", "rate limited", {
      cause: new Error("429"),
    });
