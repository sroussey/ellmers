  ...
  implements IVectorStorage<Metadata, Schema, Entity, PrimaryKeyNames>
{
  ...
  /**
   * Atomic batch put backed by a single IDB `readwrite` transaction. The
   * inherited tabular `putBulk` opens one transaction per row via
   * `Promise.all`, so a duplicate-key or quota error on the second or later
   * record can leave earlier rows committed — breaking the all-or-nothing
   * guarantee vector ingestion (RAG chunk upserts) depends on. The
   * single-transaction path defers `put` event emission to `tx.oncomplete`,
   * which means subscribers see a successful batch as a burst rather than
   * interleaved with the per-record IDB requests.
   */
  override async putBulk(
    records: InsertEntity<Entity, AutoGeneratedKeys<Schema>>[]
  ): Promise<Entity[]> {
