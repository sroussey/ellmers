    ...
    const run = async (): Promise<Entity[]> => {
      ...
      return new Promise<Entity[]>((resolve, reject) => {
        ...
        for (let i = 0; i < prepared.length; i++) {
          ...
          // Per-request onerror is required so the IDB engine doesn't surface
          // an "uncaught request error"; the transaction-level `onerror` /
          // `onabort` is what we settle the promise on. Calling
          // `event.preventDefault()` on the request would keep the tx alive,
          // which is exactly the opposite of what we want — let the request
          // error propagate to abort the transaction.
        }
