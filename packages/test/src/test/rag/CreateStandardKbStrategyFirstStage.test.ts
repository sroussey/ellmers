...
describe("createStandardKbStrategy first-stage sizing (rerank mode)", () => {
  ...
  it("topK=10, multiplier=10, firstStageMinimum=20 → first-stage topK=100 (product wins over minimum)", async () => {
    // Locks in precedence: when `topK * firstStageMultiplier` exceeds
    // `firstStageMinimum`, the product wins. A regression that flipped the
    // Math.max arguments (or replaced `max` with `min`) would surface here.
    const firstStage = await captureFirstStageTopK(
      { firstStageMultiplier: 10, firstStageMinimum: 20 },
      10
    );
    expect(firstStage).toBe(100);
  });
});
