...
/**
 * `StreamingSupabaseTaskOutputRepository` is a durable, cloud streaming cache
 * backing: JSON rows via `SupabaseTabularStorage`, port payloads as ordered
 * `bytea` chunk rows via `TabularBlobChunkStore`. Exercised against the
 * PGlite-backed Supabase mock client (no external Supabase required).
 */
...
async function makeRepo(): Promise<{
  repo: StreamingSupabaseTaskOutputRepository;
  table: string;
}> {
...
describe("StreamingSupabaseTaskOutputRepository", () => {
  afterAll(async () => {
    await client.close();
  });
  ...
  it("round-trips a binary port through a ref (bytea) and via getOutputByRef", async () => {
    const { repo } = await makeRepo();
  ...
  it("returns undefined from both readers for unknown / foreign refs", async () => {
    const { repo } = await makeRepo();
    const unknown = makeCacheRef({ $ref: "supablob://never-written" });
  ...
  it("a sibling instance over the same client + table resolves the ref", async () => {
