...
async function makeRepo(): Promise<{
  repo: StreamingPostgresTaskOutputRepository;
  table: string;
}> {
...
describe("StreamingPostgresTaskOutputRepository", () => {
  afterAll(async () => {
  ...
  it("returns undefined from both readers for unknown / foreign refs", async () => {
    const { repo } = await makeRepo();
    const unknown = makeCacheRef({ $ref: "pgblob://never-written" });
    const foreign = makeCacheRef({ $ref: "idbblob://x" });
  ...
  it("clearOlderThan prunes blob payloads alongside rows", async () => {
    ...
    await repo.clearOlderThan(-60_000); // future cutoff => everything older
    expect(await repo.getOutputStreamByRef(ref)).toBeUndefined();
  });
