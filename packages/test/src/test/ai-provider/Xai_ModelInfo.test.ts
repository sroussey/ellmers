/**
 * @license
 * Copyright 2026 Steven Roussey <sroussey@gmail.com>
 * SPDX-License-Identifier: Apache-2.0
 */

import type { AiProviderRunFn, ModelInfoTaskOutput } from "@workglow/ai";
import { XAI, _testOnly } from "@workglow/xai/ai";
import { _testOnly as runtimeTestOnly } from "@workglow/xai/ai-runtime";
import { afterEach, beforeEach, describe, expect, it } from "vitest";

const { XAI_RUN_FNS, setXaiClientForTests } = _testOnly;

function findModelInfoRunFn(): AiProviderRunFn {
  const registration = XAI_RUN_FNS.find(({ serves }) =>
    (serves as readonly string[]).includes("model.info")
  );
  expect(registration).toBeDefined();
  return registration!.runFn as AiProviderRunFn;
}

const modelConfig = (modelName: string) =>
  ({
    model_id: modelName,
    title: modelName,
    description: "",
    provider: XAI,
    provider_config: { model_name: modelName, api_key: "test-key" },
    capabilities: ["text.generation", "model.info"],
    metadata: {},
  }) as never;

describe("Xai_ModelInfo", () => {
  let retrieved: string[];

  beforeEach(() => {
    retrieved = [];
    const fakeClient = {
      models: {
        retrieve: async (id: string) => {
          retrieved.push(id);
          if (id === "missing-model") {
            const err = new Error("Not Found") as Error & { status: number };
            err.status = 404;
            throw err;
          }
          return { id };
        },
      },
    };
    setXaiClientForTests(fakeClient);
    runtimeTestOnly.setXaiClientForTests?.(fakeClient);
  });

  afterEach(() => {
    setXaiClientForTests(undefined);
    runtimeTestOnly.setXaiClientForTests?.(undefined);
  });

  it("retrieves the model and emits a remote info finish on success", async () => {
    const runFn = findModelInfoRunFn();
    const model = modelConfig("grok-4.6");
    let data: ModelInfoTaskOutput | undefined;
    await runFn({ model }, model, undefined as never, (ev) => {
      if (ev.type === "finish") data = ev.data as ModelInfoTaskOutput;
    });

    expect(retrieved).toEqual(["grok-4.6"]);
    expect(data?.is_remote).toBe(true);
  });

  it("throws naming the id and provider when retrieve 404s", async () => {
    const runFn = findModelInfoRunFn();
    const model = modelConfig("missing-model");
    await expect(runFn({ model }, model, undefined as never, (() => {}) as never)).rejects.toThrow(
      /XAI.*missing-model/i
    );
  });
});
